...
export default {
    ...
    'privacy.status.text':
        'Die Statusseite zeigt, ob meine eigenen Dienste erreichbar sind. Sie stützt sich auf zwei Systeme, die ich selbst betreibe: meine eigenen, planmäßigen Prüfungen, die meine Deployments einmal pro Minute aufrufen (und bei einem Fehlschlag sofort noch ein zweites Mal) und das Ergebnis in meiner Datenbank speichern — Antwortzeit, erreichbar/nicht erreichbar, den HTTP-Statuscode und bei einer fehlgeschlagenen Prüfung eine kurze technische Fehlermeldung, die auf einen Netzwerk-Fehlercode wie (ECONNRESET) enden kann. Prüfergebnisse werden nach 90 Tagen gelöscht. Außerdem zeigt die Seite den Median und das 95. Perzentil der Antwortzeit der letzten 24 Stunden, berechnet aus diesen gespeicherten Antwortzeiten. Das zweite System ist Metrion, eine eigenständige Monitoring-Anwendung von mir, deren Ingest-Dienst und selbst gehostete Datenbank auf meinem Contabo-VPS laufen.\n\nMein Server ruft die Werte von Metrion über dessen öffentliche API server-seitig ab, dein Browser stellt also nie eine Anfrage an Metrion, und keine Besucher-IP erreicht es auf diesem Weg. Beide Systeme messen meine Infrastruktur, nicht dich: Diese Prüfergebnisse beschreiben meine eigenen Dienste und enthalten keine Besucherdaten.',
